...
If the value of the =name= key is a function, the data is the value
returned by the method with the given =name=. The function should be
...
the context of section overloaded with the context of the
current item).
...
Which is a sort of context overloading.

_Function_:
If the value of a section variable is a function, it will be called
in the context of the current item in the list on each iteration.
...
**** TODO False values or Empty lists
**** TODO Lambdas
